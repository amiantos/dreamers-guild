...
import { LoraCache, TiCache, GeneratedImage, Album, ImageAlbum } from '../db/models.js';
 ...
 *   - includeSmart: Include smart albums in response
 */
router.get('/', (req, res) => {
  try {
    const includeHidden = req.query.includeHidden === 'true';
    const includeSmart = req.query.includeSmart === 'true';
    ...
    // Optionally include smart albums
    if (includeSmart) {
      const showFavorites = req.query.favorites === 'true';
      const showHiddenOnly = req.query.showHiddenOnly === 'true';

      // Check cache first for smart albums
      let smartAlbums = albumCache.get(showFavorites, showHiddenOnly);
      if (!smartAlbums) {
        smartAlbums = generateAlbumsForFilters(showFavorites, showHiddenOnly);
        if (smartAlbums.length > 0) {
          albumCache.set(showFavorites, showHiddenOnly, smartAlbums);
        }
      }
...
/**
 * GET /api/albums/smart
 * Get auto-generated smart albums (clustering-based)
 */
router.get('/smart', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch smart albums' });
  }
});
...
// ============================================================================
// SMART ALBUMS (AUTO-GENERATED CLUSTERING)
// ============================================================================
