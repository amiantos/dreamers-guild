 ...
 * Used for testing to ensure isolation between tests.
 */
export function createTestDatabase() {
  const db = new Database(':memory:');
  db.pragma('journal_mode = WAL');

  // Create all tables (matching database.js schema)
  ...
  // Create indexes
  db.exec(`CREATE INDEX IF NOT EXISTS idx_images_date_created ON generated_images(date_created DESC)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_images_request_id ON generated_images(request_id)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_requests_date_created ON horde_requests(date_created DESC)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_albums_slug ON albums(slug)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_albums_hidden ON albums(is_hidden)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_image_albums_image ON image_albums(image_uuid)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_image_albums_album ON image_albums(album_id)`);
  db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_pending_downloads_request_uri ON horde_pending_downloads(request_id, uri)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_lora_cache_version ON lora_cache(version_id)`);
  db.exec(`CREATE INDEX IF NOT EXISTS idx_ti_cache_version ON ti_cache(version_id)`);

  // Create model factories bound to this database
...
/**
 * Creates model objects bound to a specific database instance.
 * Mirrors the production models but without file system operations.
 */
function createModels(db) {
  // Helper for generating slugs (simplified for tests)
  ...
  const HordeRequest = {
    create(data) {
      ...
      const stmt = db.prepare(`
        ...
        (uuid, date_created, prompt, full_request, status, message, n, queue_position, wait_time, total_kudos_cost, horde_id, finished, waiting, processing, album_id)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
  ...
  const GeneratedImage = {
    ...
    countByRequestId(requestId) {
      const stmt = db.prepare(`SELECT COUNT(*) as count FROM generated_images WHERE request_id = ?`);
    ...
    delete(uuid) {
      // In tests, we skip file deletion
  ...
  const LoraCache = {
    ...
    cleanup(maxAgeMs = 90 * 24 * 60 * 60 * 1000) {
